...
const STORAGE_KEY = 'aura_learning_session'
const LEGACY_STORAGE_KEY = 'aura_public_session'
...
export function writeSession(session: LearningSession) {
  ensureBrowser()
...
export async function loginWithPassword(input: {
  ...
  const payload = await parseJson<LearningSession>(response)
...
export async function authFetch<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  const initialToken = session?.accessToken ?? (await refreshAccessToken())
